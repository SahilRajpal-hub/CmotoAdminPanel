...
const Section = ({
  ...
  const openF = (e) => {
    ...
    setFormData({
      Carnum: popup.CarNums,
      Area: popup.Area,
      Society: popup.Society,
      date: BringDate(popup.Color),
    });
    setPhoto(null);
  };
  ...
  const SubmitPhoto = async (e) => {
    try {
      if (photo) {
        ...
        await storage
          .ref(
            `cars/${Year}/${Month}/${formData.Area}/${
              formData.Society
            }/${getTodayDate()}/${formData.Carnum}/${hour}-${min}`
          )
          .put(photo);
        let bb = await storage
          .ref(
            `cars/${Year}/${Month}/${formData.Area}/${
              formData.Society
            }/${getTodayDate()}/${formData.Carnum}`
          )
          .child(`${hour}-${min}`)
          .getDownloadURL();
        ...
        let Ref = firebase
          .database()
          .ref(`Car Status/${formData.Carnum}/Work History/${formData.date}`);
    ...
    try {
      if (formData.PhotoLink) {
        ...
        let Ref = firebase
          .database()
          .ref(
            `Car Status/${formData.Carnum}/Missed car History/${formData.date}`
          );
  ...
  return (
    <div>
      ...
      {popup.popupIsOpen && popup.form == 1 && (
        <div
          style={{
            position: "fixed",
            top: "38%",
        ...
        >
          <h1
            style={{
              textAlign: "center",
              background: `${popup.Color}` === "red" ? "red" : "white",
              color: "black",
            }}
          >
            {popup.CarNums}
      ...
      {popup.popupIsOpen && popup.form == 2 && (
        <div
          style={{
            position: "fixed",
            top: "25%",
      ...
      {popup.popupIsOpen && popup.form == 3 && (
        <div
          style={{
            position: "fixed",
            top: "20%",
